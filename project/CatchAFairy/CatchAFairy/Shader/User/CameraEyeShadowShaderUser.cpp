//=============================================================================
//	CameraEyeShadowShaderUser.cpp
//
//	カメラ視点影シェーダユーザのソースファイル
//
//	Copyright(c) 2019 Ishikawa Takayoshi All Rights Reserved.
//=============================================================================
#include "CameraEyeShadowShaderUser.h"
...
const D3D11_INPUT_ELEMENT_DESC CameraEyeShadow::INPUT_LAYOUT_DESC[INPUT_LAYOUT_COUNT] = {
	{ "SV_POSITION",	0, DXGI_FORMAT_R32G32_FLOAT,	0, 0,								D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD",		0, DXGI_FORMAT_R32G32_FLOAT,	0, D3D11_APPEND_ALIGNED_ELEMENT,	D3D11_INPUT_PER_VERTEX_DATA, 0 }
};
/// <summary>頂点シェーダファイルパス</summary>
const char* const CameraEyeShadow::VERTEX_SHADER_FILEPATH = "Resources\\Shaders\\CameraEyeShadowVertexShader.cso";
/// <summary>ジオメトリシェーダファイルパス</summary>
const char* const CameraEyeShadow::GEOMETRY_SHADER_FILEPATH = "Resources\\Shaders\\CameraEyeShadowGeometryShader.cso";
/// <summary>ピクセルシェーダファイルパス</summary>
const char* const CameraEyeShadow::PIXEL_SHADER_FILEPATH = "Resources\\Shaders\\CameraEyeShadowPixelShader.cso";


/// <summary>
/// デフォルトコンストラクタ
/// </summary>
CameraEyeShadow::CameraEyeShadow() :
	constantBufferArray{nullptr},
	topology(::CatchAFairy::Shader::User::VertexTopology::TriangleStrip),
...
CameraEyeShadow::~CameraEyeShadow(){
	...
	this->ReleaseVertexAndIndexBuffer();
}
...
/// デバイスに読み込まれた
/// </summary>
void CameraEyeShadow::OnDeviceLoaded(){
	// 基底部分を実行
...
ID3D11Buffer* CameraEyeShadow::CreateVertexBuffer(const VSInput* const _vertexArray, unsigned int _vertexCount){
...
void CameraEyeShadow::CreateSamplerState(ID3D11Device* const _device){
	// 設定
...
/// </summary>
/// <param name="_device">デバイス</param>
void CameraEyeShadow::CreateBlendState(ID3D11Device* const _device){
	// レンダーターゲットブレンド設定
...
/// </summary>
/// <param name="_device">デバイス</param>
void CameraEyeShadow::CreateDepthStencilState(ID3D11Device* const _device){
	CD3D11_DEFAULT defaultState;
...
/// </summary>
/// <param name="_device">デバイス</param>
void CameraEyeShadow::CreateRasterizerState(ID3D11Device* const _device){
	CD3D11_DEFAULT defaultState;
...
void CameraEyeShadow::DrawBefore(
	...
	// 出力リソースの初期化
	if(_color != nullptr){
		context->ClearRenderTargetView(_outputResource.shadowRenderTargetView, _color->v);
	}
...
void CameraEyeShadow::Draw(const RenderUseResource& _inputResource){
	ID3D11DeviceContext* const context = this->GetContext();
	...
	// 頂点バッファ設定
	ID3D11Buffer* vertexBuffers[1] = { this->vertexBuffer };
	...
	// インデックスバッファ設定
	context->IASetIndexBuffer(this->indexBuffer, DXGI_FORMAT_R32_UINT, 0U);
	...
	// 描画
	context->DrawIndexed(this->indexCount, 0U, 0);
...
/// 描画後処理
/// </summary>
void CameraEyeShadow::DrawAfter(){
	ID3D11DeviceContext* const context = this->GetContext();
...
/// 定数バッファ配列作成
/// </summary>
void CameraEyeShadow::CreateConstantBufferArray(){
	ID3D11Device* const device = this->GetDevice();
...
/// 定数バッファ配列開放
/// </summary>
void CameraEyeShadow::ReleaseConstantBufferArray(){
	// 一つずつ開放
...
/// 頂点バッファ、インデックスバッファ作成
/// </summary>
void CameraEyeShadow::CreateVertexAndIndexBuffer(){
	// 頂点情報
	const unsigned int vertexCount = 4U;
	::CatchAFairy::Shader::User::CameraEyeShadow::VSInput vertices[vertexCount];
	...
	// 頂点バッファ作成
	this->vertexBuffer = this->CreateVertexBuffer(vertices, vertexCount);

	// インデックス情報
	const unsigned int INDEX_COUNT = 4U;
	const unsigned long indices[INDEX_COUNT] = { 0U, 1U, 2U, 3U };
	// インデックスバッファ作成
	this->indexBuffer = this->CreateIndexBuffer(indices, INDEX_COUNT);

	this->topology = ::CatchAFairy::Shader::User::VertexTopology::TriangleStrip;
...
/// 頂点バッファ、インデックスバッファ開放
/// </summary>
void CameraEyeShadow::ReleaseVertexAndIndexBuffer(){
	// 頂点バッファ開放
	this->ReleaseBuffer(this->vertexBuffer);
	this->vertexBuffer = nullptr;

	// インデックスバッファ開放
	this->ReleaseBuffer(this->indexBuffer);
